	...
	# data for one site for one year
	# 3. Which column names appear to contain more than one variable?
	# the columns ending with j and a
	...
	#one site, year, and species stage
	# 3. Why does species_stage still violate the rules of tidy data?
	#it still contains multiple variables
	...
	#observations for one site in one year for one life stage for one species 
	...
	species_key <- tribble(
	  ...
	  "Atlantic cod", "Gadus morhua",
	  "Haddock", "Melanogrammus aeflefinus",
	  "Pollock", "Pollachius virens"
	)
