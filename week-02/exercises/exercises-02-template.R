...
water_temp_f
	...
	print(water_temp_c & water_temp_f)
	
	
	# 4. Round water_temp_f to one decimal place.
round(water_temp_f, digit = 1)
...
long_fish <- (survey_data$length_cm > 35)
long_fish	
	
	# 5. Display only the observations where a fish was caught.
was_caught <- (survey_data$caught == TRUE)	
was_caught
...
round(3.14159, digits = 2)
	...
	# of survey_data.
large_and_caught <- survey_data[survey_data$length_cm > 35 &
...
large_and_caught
	...
	#
	# Response: creating data frames has become more intuitive
